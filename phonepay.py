...
@st.cache_resource
def get_connection():
    engine = create_engine("mysql+pymysql://root:Jaya%401608@localhost:3306/phonepay")
    return engine
...
@st.cache_data
def get_states():
    # Selects 'state' (or 'states') and aliases it as 'State'
    q = "SELECT DISTINCT state AS State FROM aggregated_transaction ORDER BY state"
    df = pd.read_sql(q, engine)
    return ["All India"] + sorted(df["State"].dropna().unique().tolist())
...
with tab1:
    ...
    def get_map_data(year, quarter):
        q = """SELECT state AS State, SUM(Transaction_amount) AS Total
               FROM aggregated_transaction
               WHERE Year = %s AND Quarter = %s
               GROUP BY State"""
        df = pd.read_sql(q, engine, params=(year, quarter))
        return dict(zip(df["State"], df["Total"])), df

    @st.cache_data
    def get_statewise_transaction_categories(year, quarter):
        query = """
            SELECT State, Transaction_type, SUM(Transaction_amount) AS Amount
            FROM aggregated_transaction
            WHERE Year = %s AND Quarter = %s
            GROUP BY State, Transaction_type
        """
        df = pd.read_sql(query, engine, params=(year, quarter))
        return df.pivot(index="State", columns="Transaction_type", values="Amount").fillna(0)

    @st.cache_data
    def get_user_totals(year, quarter):
        q = """SELECT State, SUM(RegisteredUser) AS Registered, SUM(AppOpens) AS Opens
               FROM map_user
               WHERE Year = %s AND Quarter = %s
               GROUP BY State"""
        return pd.read_sql(q, engine, params=(year, quarter))
    ...
    for f in geojson["features"]:
        ...
        if data_type == "Transactions" and state in category_df.index:
            ...
            state_data = user_df[user_df["State"] == state]
        ...
        if data_type == "Transactions":
            ...
            with col_states:
                ...
                for idx, row in top_states_df.iterrows():
                    st.markdown(f"- **{row['State']}** : {row['Total']}")
            ...
            if selected_region != "All India":
                state_df = df_users[df_users["State"] == selected_region]
            ...
            st.markdown("######  Top 10 State by Total Users")
            ...
            for idx, row in df_top_states.iterrows():
                st.markdown(f"- **{row['State']}** : {int(row['Total']):,} users")
...
with tab2:
    ...
    def show_case_study_1():
        ...
        # Top  State
        st.markdown("###  Top  State by Transaction Amount")
        if selected_state == "All India":
            top_states_df = map_df.sort_values("TotalAmount", ascending=False).head(10)
        else:
            top_states_df = map_df[map_df["State"] == selected_state]

        fig_bar = px.bar(top_states_df, x="State", y="TotalAmount", color="State",
                         text_auto=".2s", title="Top Transaction State")
        ...
        if selected_state == "All India":
            trend_query = """
                SELECT CONCAT(Year, '-Q', Quarter) AS QuarterLabel,
    ...
    def show_case_study_2():
        ...
        # AppOpens vs Registered Users
        st.markdown("### App Opens vs Registered Users")
        if selected_state == "All India":
            user_query = """
                SELECT State, SUM(RegisteredUser) AS Registered, SUM(AppOpens) AS Opens
                FROM map_user
                WHERE Year = %s AND Quarter = %s
                GROUP BY State
            """
        ...
        if selected_state == "All India":
            trend_query = """
                SELECT CONCAT(Year, '-Q', Quarter) AS QuarterLabel, Brands, SUM(Transaction_count) AS Count
                FROM aggregated_user
                GROUP BY Year, Quarter, Brands
                ORDER BY Year, Quarter
            """
            trend_df = pd.read_sql(trend_query, engine)
        else:
    ...
    def show_case_study_3():
        ...
        map_query = """
            SELECT State AS State, SUM(Insurance_count) AS TransactionCount, SUM(Insurance_amount) AS TotalAmount
            FROM map_insurance 
            WHERE Year = %s AND Quarter = %s 
            GROUP BY State
        """
        map_df = pd.read_sql(map_query, engine, params=(cs3_year, cs3_quarter))
        st.markdown("### Insurance Transactions Across State")
        fig_map = px.choropleth(
            map_df,
            geojson=geojson,
            featureidkey='properties.ST_NM',
        ...
        # Top State by Insurance
        top_query = """
            SELECT State AS State, SUM(Insurance_count) AS TransactionCount 
            FROM top_insurance 
            WHERE Year = %s AND Quarter = %s 
            GROUP BY State 
        ...
        st.markdown("###  Top  State by Insurance Adoption")
        fig_bar = px.bar(
            top_df,
            x='TransactionCount',
            y='State',
            orientation='h',
            color='TransactionCount',
            title='Top  State by Insurance Transactions'
        )
        st.plotly_chart(fig_bar, use_container_width=True)
        ...
        user_query = """
            SELECT State AS State, SUM(RegisteredUser) AS RegisteredUsers 
            FROM map_user 
            WHERE Year = %s AND Quarter = %s 
            GROUP BY State
        """
        insurance_query = """
            SELECT State AS State, SUM(Insurance_count) AS InsuranceTransactions
            FROM map_insurance 
            WHERE Year = %s AND Quarter = %s 
            GROUP BY State
        """
    ...
    def show_case_study_4():
        ...
        # Choropleth Map 
        map_query = """
            SELECT State AS State, SUM(Transaction_amount) AS TotalAmount 
            FROM map_transaction 
            WHERE Year = %s AND Quarter = %s 
            GROUP BY State
        """
        ...
        trend_query = """
            SELECT Year, Quarter, SUM(Transaction_amount) AS TotalAmount 
            FROM aggregated_transaction 
            GROUP BY Year, Quarter 
            ORDER BY Year, Quarter
        """
        ...
        trend_df['Period'] = trend_df['Year'] + "-Q" + trend_df['Quarter']
        ...
        #  Volume vs Count 
        bubble_query = """
            SELECT State AS State, SUM(Transaction_amount) AS TotalAmount, SUM(Transaction_count) AS TransactionCount 
            FROM map_transaction 
            WHERE Year = %s AND Quarter = %s 
            GROUP BY State
        """
    ...
    def show_case_study_5():
        ...
        user_map_query = """
            SELECT State AS State, SUM(RegisteredUser) AS TotalRegistered
            FROM map_user
            WHERE Year = %s AND Quarter = %s
            GROUP BY State
        """
        ...
        ratio_query = """
            SELECT State AS State, SUM(RegisteredUser) AS Registered, SUM(AppOpens) AS Opens
            FROM map_user
            WHERE Year = %s AND Quarter = %s
            GROUP BY State
        """
        ratio_df = pd.read_sql(ratio_query, engine, params=(cs5_year, cs5_quarter))
        ratio_df["EngagementRatio"] = (ratio_df["Opens"] / ratio_df["Registered"].replace(0, pd.NA)).fillna(0).round(2)
        fig_bar = px.bar(
            ratio_df.sort_values("EngagementRatio", ascending=False).head(10),
            x="State",
            y="EngagementRatio",
            color="EngagementRatio",
            title="Top State by App Opens per Registered User"
        )
        st.plotly_chart(fig_bar, use_container_width=True)
